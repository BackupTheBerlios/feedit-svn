...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	LRESULT OnFileExportOPML(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if(fd.DoModal() == IDOK)
		{
			...
			MSXML2::IXMLDOMNodePtr root = xmldocument->createElement("opml");
			ATLASSERT(root != NULL);
			xmldocument->appendChild(root);
			MSXML2::IXMLDOMNodePtr node1 = xmldocument->createElement("head");
			ATLASSERT(node1 != NULL);
			MSXML2::IXMLDOMNodePtr node2 = xmldocument->createElement("title");
			ATLASSERT(node2 != NULL);
			node2->text = "Generated by FeedIt";
			node1->appendChild(node2);
			root->appendChild(node1);
			node1 = xmldocument->createElement("body");
			ATLASSERT(node1 != NULL);
			m_connection.CreateInstance(__uuidof(ADODB::Connection));
			ATLASSERT(m_connection != NULL);
			m_connection->Open(_bstr_t("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=")+m_dbPath, _bstr_t(), _bstr_t(), 0);
			...
			if(!recordset->EndOfFile)
			{
				...
				while(!recordset->EndOfFile)
				{
					node2 = xmldocument->createElement("outline");
					ATLASSERT(node2 != NULL);
					MSXML2::IXMLDOMNodePtr attr = xmldocument->createAttribute("title");
					ATLASSERT(attr != NULL);
					attr->text = (_bstr_t)recordset->Fields->GetItem("Name")->Value;
					...
					subcommand->ActiveConnection = m_connection;
					subcommand->CommandText = "SELECT * FROM Feeds WHERE FolderID=?";
					subcommand->GetParameters()->Append(subcommand->CreateParameter(_bstr_t(), ADODB::adInteger, ADODB::adParamInput, NULL, recordset->Fields->GetItem("ID")->Value));
					ADODB::_RecordsetPtr subrecordset = subcommand->Execute(NULL, NULL, 0);
					...
					if(!subrecordset->EndOfFile)
					{
						...
						while(!subrecordset->EndOfFile)
						{
							MSXML2::IXMLDOMNodePtr node3 = xmldocument->createElement("outline");
							ATLASSERT(node3 != NULL);
							...
							ATLASSERT(attr != NULL);
							attr->text = (_bstr_t)subrecordset->Fields->GetItem("Title")->Value;
							node3->attributes->setNamedItem(attr);
							attr = xmldocument->createAttribute("description");
							ATLASSERT(attr != NULL);
							attr->text = (_bstr_t)subrecordset->Fields->GetItem("Description")->Value;
							node3->attributes->setNamedItem(attr);
							attr = xmldocument->createAttribute("xmlUrl");
							ATLASSERT(attr != NULL);
							attr->text = (_bstr_t)subrecordset->Fields->GetItem("URL")->Value;
							node3->attributes->setNamedItem(attr);
							attr = xmldocument->createAttribute("htmlUrl");
							ATLASSERT(attr != NULL);
							attr->text = (_bstr_t)subrecordset->Fields->GetItem("Link")->Value;
							node3->attributes->setNamedItem(attr);
			...
			if(!subrecordset->EndOfFile)
			{
				...
				while(!subrecordset->EndOfFile)
				{
					MSXML2::IXMLDOMNodePtr node3 = xmldocument->createElement("outline");
					ATLASSERT(node3 != NULL);
					MSXML2::IXMLDOMNodePtr attr = xmldocument->createAttribute("type");
					ATLASSERT(attr != NULL);
					attr->text = "rss";
					node3->attributes->setNamedItem(attr);
					attr = xmldocument->createAttribute("title");
			...
			root->appendChild(node1);
			xmldocument->save(fd.m_szFileName);
		}
	...
	LRESULT OnActionsOpenInBrowser(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		int idx = m_listView.GetSelectedIndex();
		NewsData* newsdata = dynamic_cast<NewsData*>((ListData*)m_listView.GetItemData(idx));

		if(newsdata != NULL)
			ShellExecute(NULL, "open", newsdata->m_url, NULL, NULL, SW_SHOWNORMAL);

		return 0;
	}

	LRESULT OnActionsToggleUnread(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		int idx = m_listView.GetSelectedIndex();
		NewsData* newsdata = dynamic_cast<NewsData*>((ListData*)m_listView.GetItemData(idx));
	...
	LRESULT OnActionsToggleFlagged(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		int idx = m_listView.GetSelectedIndex();
		NewsData* newsdata = dynamic_cast<NewsData*>((ListData*)m_listView.GetItemData(idx));
